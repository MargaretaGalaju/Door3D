...
export class RecomendationsComponent {
  ...
  public romanToInt(s: string) {
    ...
    Object.keys(Combinations).filter((a)=> {console.log(Number(a))}).forEach((combination) => {
        ...
        if (s.includes(combination)) {
             calc.push({
                 symbol: combination,
                 value: Combinations[combination]
             })
         }
    });
     
      Object.keys(RomanNumbers).filter((a)=> typeof Number(a) !== 'number').forEach((combination) => {
        console.log(s.includes(combination), combination, s)
          console.log('calc')
     ...
     return calc.reduce((p, agg)=> p.value+agg, 0)
